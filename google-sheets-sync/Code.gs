/**
 * Yared Hymn Tracker — Google Sheets two-way sync
 * ------------------------------------------------
 * Paste this whole file into Extensions > Apps Script of your Google Sheet,
 * run "setup" once, then Deploy > New deployment > Web app
 * (Execute as: Me, Who has access: Anyone). Put the Web app URL and the
 * secret (menu: Yared Sync > Show app connection info) into the app.
 *
 * Columns can be reordered or added to — the script finds them by header name.
 * To delete a hymn from the sheet side, tick its "deleted" box (don't delete
...
function onOpen(){
  SpreadsheetApp.getUi().createMenu('Yared Sync')
    .addItem('Set up / repair sheet', 'setup')
    .addItem('Show app connection info', 'showInfo')
    .addItem('Refresh lyric previews', 'refreshPreviews')
    .addToUi();
}
...
function showInfo(){
  ...
  if(!url){
    try{
      ...
      if(/\/exec$/.test(u)) url = u;
    }catch(e){}
  }
  const t = HtmlService.createHtmlOutput(
    '<div style="font-family:sans-serif;font-size:14px;line-height:1.45">' +
    '<p style="margin:0 0 6px"><b>1.</b> Web app URL (from Deploy &gt; Manage deployments, ends in <code>/exec</code>):</p>' +
    '<input id="url" style="width:100%;box-sizing:border-box;padding:7px;font-size:13px" placeholder="https://script.google.com/macros/s/.../exec" value="' + url.replace(/"/g,'') + '">' +
    '<p style="margin:10px 0 4px"><b>2.</b> In the app: Home &rarr; Google Sheets sync &rarr; <b>Scan QR</b></p>' +
    '<div id="qr" style="text-align:center;margin:8px 0;min-height:40px"></div>' +
    '<p id="hint" style="color:#a33;font-size:12.5px;margin:4px 0"></p>' +
    '<p style="margin:6px 0;font-size:12.5px;color:#555">Or type it in: secret key <code style="font-size:14px;color:#000">' + getSecret_() + '</code></p>' +
    '<p style="margin:6px 0 0;font-size:12px;color:#777">This QR contains your secret key &mdash; only scan it with your own devices. Lyric images are stored in the Drive folder "' + FOLDER_NAME + '".</p>' +
    '</div>' +
    '<script>' + QR_LIB + '</' + 'script>' +
    '<script>' +
    'var SECRET=' + JSON.stringify(getSecret_()) + ';' +
    'function draw(){var u=document.getElementById("url").value.trim();var h=document.getElementById("hint");var box=document.getElementById("qr");' +
    'if(u.indexOf("https://script.google.com/")!==0||u.slice(-5)!=="/exec"){box.innerHTML="";h.textContent=u?"That doesn\u2019t look like a web app URL (it should end in /exec).":"Paste the web app URL above to show the QR code.";return;}' +
    'h.textContent="";var q=qrcode(0,"M");q.addData("YARED1|"+u+"|"+SECRET);q.make();box.innerHTML=q.createSvgTag({cellSize:5,margin:3});' +
    'google.script.run.saveWebAppUrl(u);}' +
    'document.getElementById("url").addEventListener("input",draw);draw();' +
    '</' + 'script>'
  ).setWidth(420).setHeight(560);
  SpreadsheetApp.getUi().showModalDialog(t, 'Yared Sync — connect the app');
}

function saveWebAppUrl(url){
  if(/^https:\/\/script\.google\.com\/.+\/exec$/.test(String(url)))
...
function onEdit(e){
  try{
    ...
    const map = headerMap_(sheet);
    const c0 = e.range.getColumn(), nCols = e.range.getNumColumns();
    let meaningful = false;
    for(let c = c0; c < c0 + nCols; c++){
      const name = Object.keys(map).find(k => map[k] === c - 1);
      if(name && SYSTEM_COLS.indexOf(name) === -1) meaningful = true;
    }
    ...
    for(let i = 0; i < vals.length; i++){
      ...
      if(!title) continue;
      if(!row[map.id]) sheet.getRange(start + i, map.id + 1).setValue(newId_());
      sheet.getRange(start + i, map.updatedAt + 1).setValue(now);
      setPreview_(sheet, map, start + i, String(row[map.lyricImageLinks] || ''));
    }
...
function doPost(e){
  ...
  if(body.secret !== getSecret_()) return json_({ok:false, error:'Wrong secret key'});
...
function image_(id, url){
  let blob;
  if(id){
    blob = DriveApp.getFileById(id).getBlob();
  } else if(url){
    const res = UrlFetchApp.fetch(url, {followRedirects:true, muteHttpExceptions:true});
    if(res.getResponseCode() >= 400) return {ok:false, error:'Could not fetch image (' + res.getResponseCode() + ')'};
    blob = res.getBlob();
  } else {
    return {ok:false, error:'No image id or url'};
  }
...
function headerMap_(sheet){
  ...
  const map = {};
  hdr.forEach((h, i) => { if(h && map[h] === undefined) map[h] = i; });
  return map;
}
...
function getSecret_(){
  ...
  if(!s){
    s = Utilities.getUuid().replace(/-/g, '').slice(0, 12);
